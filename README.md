...
The file `detection.txt` in the output folder records the methylation score of every GpC site on each molecule, which is essentially the posterior probability of methylation. The first two columns are the coordinates of GpC site. The fourth column is the read name. The last column is the methylation score.
